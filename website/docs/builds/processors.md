...
Processor blocks 1 through 8 are fixed, every theme can replace them.  
Processor blocks 9 through 14 are extra, not fixed, and a seperate processor has to be provided, specific to that room. That processor runs along additionally to the standard processor, so both pools can be used.  
